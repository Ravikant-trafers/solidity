...
value in the constructor of the contract. There can only be a single assignment
and it cannot be read from during construction time.
The compiler actually modifies the code by writing that value into the runtime
code before it returns it. This is important if you are comparing the
